 ...
 *
 * Background: the original refresh batch wrote `lastFetchAttemptAt = now`
 ...
 *
 * This script clears `lastFetchAttemptAt` for a known list of company names,
...
async function main() {
  ...
  const toReset = matches.filter(
    (c) => c.indeedRating == null && c.indeedUrl == null && c.lastFetchAttemptAt != null,
  );
  ...
  if (skipped.length > 0) {
    ...
    for (const c of skipped) {
      console.log(
        `  - ${c.name} (rating=${c.indeedRating}, url=${c.indeedUrl ? 'yes' : 'no'}, attempted=${c.lastFetchAttemptAt ?? 'null'})`,
      );
    }
    console.log();
  }
  ...
  console.log(`Will clear lastFetchAttemptAt on ${toReset.length} companies:`);
